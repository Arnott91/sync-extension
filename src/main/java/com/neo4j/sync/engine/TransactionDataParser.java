...
public class TransactionDataParser {
    ...
    private static Map<String, Object> getKeyValueComponents(JSONObject event, ParseType parseType) throws JSONException {
        ...
        switch (parseType) {
            case NODE_PROPERTIES:
                return getNodeProperties(event);
            case PRIMARY_KEY:
                return getPrimaryKey(event);
            case REL_PROPERTIES:
                return getRelationProperties(event);
            default:
                return null;
    ...
    public static Map<String, Object> getPrimaryKey(JSONObject nodeEvent, NodeDirection direction) throws JSONException {
        switch (direction) {
            case START:
                return ((JSONObject) nodeEvent.get(PRIMARY_KEY1)).toMap();
            case TARGET:
                return ((JSONObject) nodeEvent.get(TARGET_PRIMARY_KEY)).toMap();
            default:
                return null;
        }
    }
